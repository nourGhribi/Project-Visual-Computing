...
class QuadGraph {
  ...
  void displayQuadLines(List<PVector> corners){
    stroke(204,102,0);
    for(int i = 0; i < corners.size(); i++){
    PVector edge1 = corners.get(i);
    PVector edge2 =  corners.get((i + 1) % corners.size());
    line(edge1.x, edge1.y, edge2.x, edge2.y);
    }
  }
  void displayCorners(List<PVector> corners){
   fill(204,102,0);
   for(int i = 0; i < corners.size(); i++){
     ellipse(corners.get(i).x,corners.get(i).y,10,10);
   }
  }
